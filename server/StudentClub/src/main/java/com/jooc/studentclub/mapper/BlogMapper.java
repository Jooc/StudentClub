...
public interface BlogMapper {
    ...
    @Select("select Blog.id, post_time, url, publisher_id, User.name, User.avatar, Blog.privilege, tags from Blog join User on (Blog.publisher_id=User.id) order by post_time desc;")
    List<DBBlogModel> getAllBlog();

    @Select("select Blog.id, post_time, url, publisher_id, User.name, User.avatar, Blog.privilege, tags from Blog join User on (Blog.publisher_id=User.id) where Blog.id = #{id};")
    DBBlogModel getById(int id);

    @Select("select Blog.id, post_time, url, publisher_id, User.name, User.avatar, Blog.privilege, tags from Blog join User on (Blog.publisher_id=User.id) where Blog.privilege <= #{privilege};")
